...
get "/" do
    if !options["api_token"] || !options["api_secret"] || !options["domain"] || !options["user_id"]
      ...
      application = (Application.list(client).select {|a| a.name == APPLICATION_NAME}).first ||
          Applcation.create(client, :name => APPLICATION_NAME, :incoming_call_url => callback_url, :auto_answer => false)
      numbers = (PhoneNumber.list(client).select {|p| p.application.end_with?("/#{application.id}")})
      if numbers.size < 2
        available_numbers = AvailableNumber.search_local(:city => "Cary", :state => "NC", :quantity => 2)
        numbers = available_numbers.map {|n| PhoneNumber.create(:number => n[:number], :application_id => application.id)}
      end
      ...
      endpoint = (domain.get_endpoints().map {|p| p.name == USER_NAME && p.application_id == application.id}).first ||
...
post "/events/calls" do
  ...
  case(params["eventType"])
    when "incomingcall"
      callback_url = "http://#{options["domain"]}/event/bridged"
